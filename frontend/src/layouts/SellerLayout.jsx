...
import { LayoutDashboard, Package, ShoppingBag, Wallet, Settings, ExternalLink } from 'lucide-react';
...
export function SellerLayout() {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/seller/dashboard', icon: <LayoutDashboard className="w-4 h-4 mr-2" /> },
    { name: 'Products', path: '/seller/products', icon: <Package className="w-4 h-4 mr-2" /> },
    { name: 'Orders', path: '/seller/orders', icon: <ShoppingBag className="w-4 h-4 mr-2" /> },
    { name: 'Payouts', path: '/seller/payouts', icon: <Wallet className="w-4 h-4 mr-2" /> },
    { name: 'Settings', path: '/seller/settings', icon: <Settings className="w-4 h-4 mr-2" /> },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-8">
              <Link to="/seller/dashboard" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                ...
                <span className="font-bold tracking-tight text-lg">Seller Center</span>
              </Link>
            ...
            <div className="flex items-center gap-4 text-sm relative">
              ...
              >
                <span className="text-foreground font-bold hidden sm:block">{user?.email || 'seller@audiohub.com'}</span>
                <div className="w-8 h-8 rounded-lg bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/10 flex items-center justify-center text-foreground font-bold">S</div>
              </button>
              ...
              {isDropdownOpen && (
                <div className="absolute top-full right-0 mt-2 w-56 bg-white dark:bg-[#18181b] border border-black/10 dark:border-white/10 rounded-2xl shadow-xl overflow-hidden z-50">
                  <div className="p-4 border-b border-black/5 dark:border-white/5 md:hidden">
                    <p className="font-bold text-sm truncate">{user?.email || 'seller@audiohub.com'}</p>
                  </div>
                  ...
                  <div className="p-2">
                    <Link to="/seller/onboarding" onClick={() => setIsDropdownOpen(false)} className="block px-3 py-2 text-foreground hover:bg-black/5 dark:hover:bg-white/5 rounded-lg text-sm font-medium transition-colors">Onboarding</Link>
                  </div>
